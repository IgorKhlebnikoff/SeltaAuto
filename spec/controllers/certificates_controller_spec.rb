...
RSpec.describe CertificatesController, :type => :controller do
  let!(:photo) { Certificate.create(asset_file_name: "some name") }
  describe "#index" do
    before {get :index}
  ...
  describe "#new" do
    ...
    it "sets a new photo" do
      expect(assigns(:photo)).to be_kind_of(Certificate)
    end
  end
  ...
  describe "#create" do
    ...
    it "redirects to certificates" do
      post :create, certificate: {asset_file_name: "some name"}
  ...
  describe "#update" do
    let!(:certificate) { Certificate.create(asset_file_name: "some name") }
    before do
      controller.stub(:authenticate_user!).and_return(true)
    end
    it "redirects to certificates" do
      put :update, id: certificate, certificate: {asset_file_name: "some second name"}
      expect(response).to redirect_to certificates_path
    end
    it "return status 302" do
      put :update, id: certificate, certificate: {asset_file_name: "some second name"}
